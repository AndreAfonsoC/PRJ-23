...
from aux_tools import atmosphere, geo_change_sweep

# Nao foram dados os parametros:
# y_n da nacelle
# z_n da nacelle
# xcg_0 da fuselagem
# xnp da fuselagem
# Foram estimados de forma que o plot final ficasse proximo ao presente no roteiro do lab
...
def hermes_aircraft():
    # Defining general geometric parameters
    wing = {'S':1.613e+02,  # Otimização
    		'AR': 8.032e+00,  # Otimização
    		'taper':0.15, # para caber tanque na asa 
    		'sweep':3.422e-01, #20*np.pi/180, # Otimização
    		'dihedral':7.5*np.pi/180,   # Clearance: motor não bater no chão
    		'xr':15.9,  # Otimização
    		'zr':-1.56,   
    		'tcr': 0.14,
    		'tct': 0.096,
    ...
    flap = {'max_def': 40/180*np.pi,
    		'type': 'triple slotted',
    		'c_flap_c_wing': 1.3,   # Perguntar professor
    		'b_flap_b_wing': 0.6,}  # Perguntar professor
    
    slat = {'max_def': 20/180*np.pi,
    		'type': 'slat',
    		'c_slat_c_wing': 1.1,  # Perguntar professor
    		'b_slat_b_wing': 0.75}  # Perguntar professor
    
    # Vamos assumir que vai ser o CFM-56
    engines = {'n': 2,
     		   'n_uw': 2,   # Conferir com o professor 
               'BPR': 6.4}  # BPR do A-320
    ...
    # 170 passageiros e 6 de tripulação (piloto + copiloto + 4 comissários de bordo)
    # Piloto: 240 lb
    # Comissário: 210  lb
    # Passageiro: 195 lb
    weights = {'W_payload': lb2N*220*170,
               'xcg_payload': 20.5, # Conferir com o professor (meio da cabine)
               'W_crew': lb2N*(240*2+210*4),
               'xcg_crew': 18.5, # Conferir com o professor (2 piloto + 1 aeromoça + 3 aeromoça)
               'per_xcg_allelse': 0.49} # Conferir com o professor 
    ...
    # Estimativa via motor escolhido: CFM-56
    nacelle = {'Ln': 2.5,
     		   'Dn': 1.6,
               'xn': 16.1,
               'yn': 6, # Conferir com o professor (consideramos o meio da nacelle)
               'zn': -1.67} 
    
    aircraft['dimensions']['nacelle'] = nacelle
    
    ldg = {'xnlg': 5,       # média entre B737-800 e A-320
           'xmlg': 21.69,      # média das porcentagens entre B737-800 e A-320     
           'ymlg': 4.5,       # Tem que estar entre a fuselagem e a nacele; embaixo da asa; mais perto da nacelle
           'z': -3.5}         # Fizemos para ter altura parecida com A-320
...
def geometry(aircraft):
    ...
    ## Definindo variaveis auxiliares vindas da aeronave
    # Parametros da asa
    ...
    Lc_v = aircraft['geo_param']['EV']['Lb']
    zr_v = aircraft['geo_param']['EV']['zr']

    ## Calculando os parametros fisicos da aeronave
    # Parametros fisicos da asa
    ...
    # Parametros fisicos da empenagem horizontal
    L_h = Lc_h*cm_w
    ...
    # Parametros fisicos da empenagem vertical
    L_v = Lc_v*b_w
    S_v = S_w*b_w/L_v*Cvt
